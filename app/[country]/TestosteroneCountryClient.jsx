        ...
        <div className={`relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${heroPadding}`}>
          <div className="max-w-4xl mx-auto lg:mx-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-white/80 mb-4 sm:mb-6 overflow-x-auto pb-2 whitespace-nowrap scrollbar-hide">
              <Link href="/" className="hover:text-white transition-colors cursor-pointer flex-shrink-0">
                Home
              </Link>
              ...
              <span className="text-white/60 flex-shrink-0">•</span>
              <span className="text-white font-medium bg-white/10 px-2 sm:px-3 py-1 rounded-full backdrop-blur-sm border border-white/20 flex-shrink-0">
                {data.name || countrySlug}
              </span>
            </div>
            
            {/* HERO TITLE - FIXED: No cropping, proper stacking */}
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-light mb-4 sm:mb-6">
              {hero.title ? (
                typeof hero.title === 'object' ? (
                  <div className="flex flex-col items-start gap-1 sm:gap-2">
                    {/* Prefix - Normal text */}
                    <span className="text-white text-lg sm:text-xl md:text-2xl lg:text-3xl leading-tight">
                      {hero.title.prefix}
                    </span>
                    
                    {/* Highlight - BIG BOLD text with NO CROPPING */}
                    <span className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl pb-5 font-bold text-white py-1">
            ...
            {/* Animated Subtitle - Bullet points */}
                ...
                {hero.animatedSubtitle.split('•').map((item, index) => (
                  item.trim() && (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <span className="text-sm sm:text-base md:text-lg text-white/90 font-light">
                        {item.trim()}
                      </span>
                    </div>
                  )
            ...
            content.sections.map((section, idx) => {
              ...
              return (
                <section key={idx} className="relative group">
                  <div className="absolute -left-4 sm:-left-6 top-0 w-0.5 sm:w-1 h-16 sm:h-20 lg:h-24 bg-gradient-to-b from-slate-300 to-slate-200 rounded-full opacity-0 group-hover:opacity-100 group-hover:from-blue-600 group-hover:to-blue-400 transition-all duration-500" />
