  ...
  {hero.title ? (
    typeof hero.title === 'object' ? (
      <span className="flex flex-col sm:flex-row flex-wrap items-start sm:items-baseline gap-1 sm:gap-2">
        <span className="text-white/90 text-lg sm:text-xl md:text-2xl lg:text-3xl pb-1">{hero.title.prefix}</span>
        <span className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold bg-gradient-to-r from-white to-white/90 bg-clip-text text-transparent pb-2 sm:pb-3 leading-tight">
          {hero.title.highlight}
        </span>
        <span className="text-white/90 text-lg sm:text-xl md:text-2xl lg:text-3xl pb-1">{hero.title.suffix}</span>
      </span>
    ) : (
      hero.title
    )
